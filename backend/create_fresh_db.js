...
async function createFreshDatabase() {
  ...
  try {
    ...
    await connection.execute('DROP DATABASE IF EXISTS ikibina_system');
    console.log('Dropped old database (if any)');
    
    await connection.execute('CREATE DATABASE ikibina_system');
    console.log('Created fresh ikibina_system database');
    ...
    connection = await mysql.createConnection({
      ...
      user: 'root',
      password: '',
      database: 'ikibina_system'
    });
    
    console.log('Connected to fresh ikibina_system database');
